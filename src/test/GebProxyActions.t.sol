...
// import {GemJoin3, GemJoin4} from "geb-deploy/join.sol";
...
//     }
//
//     function end_prepareCoinsForRedeeming(address a, address b, uint c) public {
//         proxy.execute(dssProxyActionsEnd, abi.encodeWithSignature("prepareCoinsForRedeeming(address,address,uint256)", a, b, c));
//     }
//
//     function end_redeemETH(address a, address b, bytes32 c, uint d) public {
//         proxy.execute(dssProxyActionsEnd, abi.encodeWithSignature("redeemETH(address,address,bytes32,uint256)", a, b, c, d));
//     }
//
//     function end_redeemTokenCollateral(address a, address b, bytes32 c, uint d) public {
//         proxy.execute(dssProxyActionsEnd, abi.encodeWithSignature("redeemTokenCollateral(address,address,bytes32,uint256)", a, b, c, d));
//     }
//
...
//     DGD dgd;
//     DSValue pipDGD;
//     CollateralAuctionHouse dgdFlip;
//     GemJoin4 gntJoin;
//     GNT gnt;
//     DSValue pipGNT;
...
//         this.file(address(scoinSavingsAccountter), "DGD", "mat", uint(1500000000 ether)); // Liquidation ratio 150%
//         this.file(address(vat), bytes32("DGD"), bytes32("line"), uint(10000 * 10 ** 45));
//         scoinSavingsAccountter.poke("DGD");
//         (,,uint scoinSavingsAccount,,) = vat.ilks("DGD");
//         assertEq(scoinSavingsAccount, 50 * ONE * ONE / 1500000000 ether);
//
...
//         this.file(address(scoinSavingsAccountter), "GNT", "mat", uint(1500000000 ether)); // Liquidation ratio 150%
//         this.file(address(vat), bytes32("GNT"), bytes32("line"), uint(10000 * 10 ** 45));
//         scoinSavingsAccountter.poke("GNT");
//         (,, scoinSavingsAccount,,) = vat.ilks("GNT");
//         assertEq(scoinSavingsAccount, 100 * ONE * ONE / 1500000000 ether);
//
//         manager = new GebCdpManager(address(vat));
//         DSProxyFactory factory = new DSProxyFactory();
//         registry = new ProxyRegistry(address(factory));
//         dssProxyActions = address(new GebProxyActions());
//         dssProxyActionsEnd = address(new GebProxyActionsGlobalSettlement());
//         dssProxyActionsDsr = address(new GebProxyActionsCoinSavingsAccount());
...
//         address bag = this.makeCollateralBag(address(gntJoin));
...
//         this.lockTokenCollateral(address(manager), address(gntJoin), cdp, 2 ether, false);
...
//         this.lockETH.value(2 ether)(address(manager), address(ethJoin), cdp);
//         assertEq(dai.balanceOf(address(this)), 0);
//         this.generateDebt(address(manager), address(jug), address(daiJoin), cdp, 300 ether);
...
//         this.repayDebt(address(manager), address(daiJoin), cdp, 100 ether);
...
//         this.repayAllDebt(address(manager), address(daiJoin), cdp);
//         assertEq(dai.balanceOf(address(this)), 0);
...
//         this.generateDebt(address(manager), address(jug), address(daiJoin), cdp, 300 ether);
//         dai.approve(address(proxy), 100 ether);
//         this.safeRepayDebt(address(manager), address(daiJoin), cdp, 100 ether, address(proxy));
//         assertEq(dai.balanceOf(address(this)), 200 ether);
//         assertEq(art("ETH", manager.urns(cdp)), 200 ether);
//     }
//
...
//         this.generateDebt(address(manager), address(jug), address(daiJoin), cdp, 300 ether);
//         dai.approve(address(proxy), 300 ether);
//         this.safeRepayAllDebt(address(manager), address(daiJoin), cdp, address(proxy));
//         assertEq(dai.balanceOf(address(this)), 0);
//         assertEq(art("ETH", manager.urns(cdp)), 0);
//     }
//
...
//         dai.approve(address(proxy), 100 ether);
//         this.give(address(manager), cdp, address(123));
//         this.safeRepayDebt(address(manager), address(daiJoin), cdp, 100 ether, address(321));
//     }
//
...
//         dai.approve(address(proxy), 300 ether);
//         this.give(address(manager), cdp, address(123));
//         this.safeRepayAllDebt(address(manager), address(daiJoin), cdp, address(321));
//     }
//
...
//         this.generateDebt(address(manager), address(jug), address(daiJoin), cdp, 300 ether);
//         dai.approve(address(proxy), 300 ether);
//         this.repayDebt(address(manager), address(daiJoin), cdp, 300 ether);
//         assertEq(art("ETH", manager.urns(cdp)), 0);
//     }
//
...
//         this.repayDebt(address(manager), address(daiJoin), cdp, 300 ether * times);
//         assertEq(art("ETH", manager.urns(cdp)), 0);
//     }
//
...
//         assertEq(ink("ETH", manager.urns(cdp)), 0);
//         assertEq(dai.balanceOf(address(this)), 0);
//         this.lockETHAndGenerateDebt.value(2 ether)(address(manager), address(jug), address(ethJoin), address(daiJoin), cdp, 300 ether);
//         assertEq(ink("ETH", manager.urns(cdp)), 2 ether);
//         assertEq(dai.balanceOf(address(this)), 300 ether);
//         assertEq(address(this).balance, initialBalance - 2 ether);
//     }
...
//         uint initialBalance = address(this).balance;
//         assertEq(dai.balanceOf(address(this)), 0);
//         uint cdp = this.openLockETHAndGenerateDebt.value(2 ether)(address(manager), address(jug), address(ethJoin), address(daiJoin), "ETH", 300 ether);
//         assertEq(ink("ETH", manager.urns(cdp)), 2 ether);
//         assertEq(dai.balanceOf(address(this)), 300 ether);
//         assertEq(address(this).balance, initialBalance - 2 ether);
//     }
...
//         assertEq(ink("COL", manager.urns(cdp)), 0);
//         assertEq(dai.balanceOf(address(this)), 0);
//         this.lockTokenCollateralAndGenerateDebt(address(manager), address(jug), address(colJoin), address(daiJoin), cdp, 2 ether, 10 ether, true);
//         assertEq(ink("COL", manager.urns(cdp)), 2 ether);
...
//         assertEq(ink("DGD", manager.urns(cdp)), 0);
//         uint prevBalance = dgd.balanceOf(address(this));
//         this.lockTokenCollateralAndGenerateDebt(address(manager), address(jug), address(dgdJoin), address(daiJoin), cdp, 3 * 10 ** 9, 50 ether, true);
...
//         this.lockTokenCollateralAndGenerateDebt(address(manager), address(jug), address(gntJoin), address(daiJoin), cdp, 3 ether, 50 ether, false);
...
//         col.approve(address(proxy), 2 ether);
//         assertEq(dai.balanceOf(address(this)), 0);
//         uint cdp = this.openLockTokenCollateralAndGenerateDebt(address(manager), address(jug), address(colJoin), address(daiJoin), "COL", 2 ether, 10 ether, true);
//         assertEq(ink("COL", manager.urns(cdp)), 2 ether);
//         assertEq(dai.balanceOf(address(this)), 10 ether);
//         assertEq(col.balanceOf(address(this)), 3 ether);
//     }
...
//         gnt.transfer(bag, 2 ether);
//         uint cdp = this.openLockTokenCollateralAndGenerateDebt(address(manager), address(jug), address(gntJoin), address(daiJoin), "GNT", 2 ether, 10 ether, false);
//         assertEq(ink("GNT", manager.urns(cdp)), 2 ether);
//         assertEq(dai.balanceOf(address(this)), 10 ether);
//     }
//
...
//         (address bag, uint cdp) = this.openLockGNTAndGenerateDebt(address(manager), address(jug), address(gntJoin), address(daiJoin), "GNT", 2 ether, 10 ether);
//         assertEq(address(bag), gntJoin.bags(address(proxy)));
//         assertEq(ink("GNT", manager.urns(cdp)), 2 ether);
//         assertEq(dai.balanceOf(address(this)), 10 ether);
//     }
...
//         (address bag, uint cdp) = this.openLockGNTAndGenerateDebt(address(manager), address(jug), address(gntJoin), address(daiJoin), "GNT", 2 ether, 10 ether);
//         (address bag2, uint cdp2) = this.openLockGNTAndGenerateDebt(address(manager), address(jug), address(gntJoin), address(daiJoin), "GNT", 2 ether, 10 ether);
//         assertEq(address(bag), gntJoin.bags(address(proxy)));
...
//         this.repayDebtAndFreeETH(address(manager), address(ethJoin), address(daiJoin), cdp, 1.5 ether, 250 ether);
...
//         this.lockETHAndGenerateDebt.value(2 ether)(address(manager), address(jug), address(ethJoin), address(daiJoin), cdp, 300 ether);
//         dai.approve(address(proxy), 300 ether);
//         this.repayAllDebtAndFreeETH(address(manager), address(ethJoin), address(daiJoin), cdp, 1.5 ether);
//         assertEq(ink("ETH", manager.urns(cdp)), 0.5 ether);
//         assertEq(art("ETH", manager.urns(cdp)), 0);
//         assertEq(dai.balanceOf(address(this)), 0);
//         assertEq(address(this).balance, initialBalance - 0.5 ether);
...
//         this.repayDebtAndFreeTokenCollateral(address(manager), address(colJoin), address(daiJoin), cdp, 1.5 ether, 8 ether);
...
//         this.repayAllDebtAndFreeTokenCollateral(address(manager), address(colJoin), address(daiJoin), cdp, 1.5 ether);
//         assertEq(ink("COL", manager.urns(cdp)), 0.5 ether);
...
//         this.repayDebtAndFreeTokenCollateral(address(manager), address(dgdJoin), address(daiJoin), cdp, 1 * 10 ** 9, 25 ether);
//         assertEq(ink("DGD", manager.urns(cdp)), 2 ether);
...
//         this.denyCDPModification(address(vat), address(123));
//         assertEq(vat.can(address(proxy), address(123)), 0);
//     }
//
//     function testQuit() public {
//         uint cdp = this.open(address(manager), "ETH", address(proxy));
//         this.lockETHAndGenerateDebt.value(1 ether)(address(manager), address(jug), address(ethJoin), address(daiJoin), cdp, 50 ether);
//
//         assertEq(ink("ETH", manager.urns(cdp)), 1 ether);
//         assertEq(art("ETH", manager.urns(cdp)), 50 ether);
...
//         this.lockETHAndGenerateDebt.value(1 ether)(address(manager), address(jug), address(ethJoin), address(daiJoin), cdpSrc, 50 ether);
//
...
//         this.lockETHAndGenerateDebt.value(1 ether)(address(manager), address(jug), address(ethJoin), address(daiJoin), cdp, 200 ether); // Maximun DAI generated
//         pipETH.poke(bytes32(uint(300 * 10 ** 18 - 1))); // Force liquidation
//         scoinSavingsAccountter.poke("ETH");
...
//         this.lockTokenCollateralAndGenerateDebt(address(manager), address(jug), address(colJoin), address(daiJoin), cdp, 1 ether, 40 ether, true);
//
//         pipCOL.poke(bytes32(uint(40 * 10 ** 18))); // Force liquidation
//         scoinSavingsAccountter.poke("COL");
...
//         this.lockTokenCollateralAndGenerateDebt(address(manager), address(jug), address(dgdJoin), address(daiJoin), cdp, 1 * 10 ** 9, 30 ether, true);
//
...
//         col.mint(1 ether);
//         col.approve(address(proxy), 1 ether);
//         uint cdp2 = this.openLockTokenCollateralAndGenerateDebt(address(manager), address(jug), address(colJoin), address(daiJoin), "COL", 1 ether, 5 ether, true);
//         dgd.approve(address(proxy), 1 * 10 ** 9);
//         uint cdp3 = this.openLockTokenCollateralAndGenerateDebt(address(manager), address(jug), address(dgdJoin), address(daiJoin), "DGD", 1 * 10 ** 9, 5 ether, true);
//
...
//         this.end_prepareCoinsForRedeeming(address(daiJoin), address(end), 310 ether);
//
//         this.end_redeemETH(address(ethJoin), address(end), "ETH", 310 ether);
//         this.end_redeemTokenCollateral(address(colJoin), address(end), "COL", 310 ether);
//         this.end_redeemTokenCollateral(address(dgdJoin), address(end), "DGD", 310 ether);
//
...
//         this.file(address(coinSavingsAccount), "dsr", uint(1.05 * 10 ** 27)); // 5% per second
...
//         this.denyCDPModification(address(vat), address(daiJoin)); // Remove vat permission for daiJoin to test it is correctly re-activate in exit
//         this.dsr_join(address(daiJoin), address(coinSavingsAccount), 50 ether);
...
//         coinSavingsAccount.drip();
//         assertEq(coinSavingsAccount.pie(address(proxy)) * coinSavingsAccount.chi(), 52.5 ether * ONE); // Now the equivalent DAI amount is 2.5 DAI extra
//         this.dsr_exit(address(daiJoin), address(coinSavingsAccount), 52.5 ether);
...
//         this.file(address(coinSavingsAccount), "dsr", uint(1.05 * 10 ** 27));
...
//         coinSavingsAccount.drip(); // Just necessary to check in this test the updated value of chi
...
//         this.file(address(coinSavingsAccount), "dsr", uint(1.03434234324 * 10 ** 27));
//         uint initialTime = 1;
//         hevm.warp(initialTime);
//         uint cdp = this.open(address(manager), "ETH", address(proxy));
//         this.lockETHAndGenerateDebt.value(1 ether)(address(manager), address(jug), address(ethJoin), address(daiJoin), cdp, 50 ether);
...
//         this.dsr_exit(address(daiJoin), address(coinSavingsAccount), 50 ether);
...
//         uint cdp = this.open(address(manager), "ETH", address(proxy));
//         this.lockETHAndGenerateDebt.value(1 ether)(address(manager), address(jug), address(ethJoin), address(daiJoin), cdp, 50 ether);
//         this.denyCDPModification(address(vat), address(daiJoin)); // Remove vat permission for daiJoin to test it is correctly re-activate in exitAll
//         dai.approve(address(proxy), 50 ether);
//         this.dsr_join(address(daiJoin), address(coinSavingsAccount), 50 ether);
//         this.dsr_exitAll(address(daiJoin), address(coinSavingsAccount));
